...
/**
 * Code borrowed from mezzio/mezzio-laminasviewrenderer
 *
...
class NamespacedPathStackResolver extends TemplatePathStack
{
    ...
    public function __construct(?iterable $options = null)
    {
        ...
        if ($this->useViewStream) {
            if (! in_array('smf.template', stream_get_wrappers())) {
                stream_wrapper_register('smf.template', Stream::class);
            }
        }
    ...
    public function resolve($name, ?RendererInterface $renderer = null): ?string
    {
        $namespace = self::DEFAULT_NAMESPACE;
        $template  = $name;
        // original - if (preg_match('#^(?P<namespace>[^:]+)::(?P<template>.*)$#', $template, $matches)) {
        ...
        foreach ($this->paths[$namespace] as $path) {
            ...
            if ($file->isReadable()) {
                ...
                if ($this->useStreamWrapper()) {
                    ...
                    $filePath = 'smf.template://' . $filePath;
                }
                return $filePath;
            }
        }
